...
class FirebaseBasicService {
    ...
    static newWithCallback(childRef,id,obj,callback){
      let pathName = "/"+childRef+"/active/"+id+"/"
      obj.uid = id;
      firebase.database().ref(pathName).set(obj);
      callback(obj);
    }
    static newWithCallbackWithoutUID(childRef,obj,callback){
      var uid = Date.now();
      let pathName = "/"+childRef+"/active/"+uid+"/"
      firebase.database().ref(pathName).set(obj);
      callback(obj);
    }
    static newWithCallbackWithoutUIDRETO(childRef,obj,callback){
      let pathName = "/"+childRef+"/active/"+obj.uid+"/"
      firebase.database().ref(pathName).set(obj);
      callback(obj);
    }
    static newWithKeyWithCallback(childRef, key, obj,callBack){
    ...
    static findActiveByIdOnce(childRef,dataId,callback,error){
        let pathName = "/"+childRef+"/"+"/active/"+dataId
        firebase.database().ref(pathName).once('value', (snapshot) => {
            let data = snapshot.val()
            if(data){
    ...
    static filterByAttributeChallenges(childRef,filter,filterData,callback,error){
        var ref = firebase.database().ref("/"+childRef+"/");
        ref.orderByChild(filter).equalTo(filterData).on('value', (snapshot) => {
            let data = snapshot.val()
            if(data){
              console.log(data)
               callback(Object.values(data).sort(function(a,b) { return parseFloat(a.uid) - parseFloat(b.uid) } ).reverse())
            }else{
              error()
            }
        })
...
static getAll(childRef,callback,error){
    return firebase.database().ref("/"+childRef+"/active/").once('value').then(function(snapshot) {
          let data = snapshot.val()
          if(data){
              callback(snapshot.val());
    ...
    static getAll(childRef,callback,error){
      var ref = firebase.database().ref("/"+childRef+"/active/");
       ref.on('value', (snapshot) => {
          let data = snapshot.val()
          if(data){
             callback(Object.values(data).sort(function(a,b) { return parseFloat(a.canton) - parseFloat(b.canton) } ).reverse());
           }else{
             error()
           }
    });
